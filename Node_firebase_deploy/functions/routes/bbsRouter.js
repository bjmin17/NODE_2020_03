...
/*
.once("value")
- firebase DB에서 데이터를 조회할 때 사용하는 이벤트 메서드
db.ret("bbs") : firebase에 데이터를 요청하는 호출 메서드
데이터 요청이 허락되면 value라는 이름으로
응답을 수행하는데
그때, 응답을 수신하는 메서드
.then(call back 함수) : callback 함수를 통해서
수신된 데이터를 처리
orderByKey() : Key 값을 기준으로 오름차순 정렬
-- DB의 Key값이 UUID 값이기 때문에 정렬하는데 별로 의미가 없다.
...
router.get("/view/:seq", (req, res, next) => {
  ...
  var db = firebase.database();
  // bbs DB에서 키 값이 seq와 같은 데이터를 찾아라
  db.ref("bbs/" + seq)
    .once("value")
    .then((resultSet) => {
...
router.get("/update/:seq", (req, res) => {
  ...
  db.ref("/bbs" + seq)
    .once("value")
    .then((result) => {
      res.render("/bbs/write", result.val());
    });
});

/*
write form을 입력, 수정에서 공유하며 사용하기 위해
새로작성을 시작하면 비어있는 bbs데이터를 만들어서
form으로 전달해주어야 한다.
