...
class App extends Component {
  ...
  */
  ...
  // Enter 키를 눌렀을 때 자동으로 추가 버튼이 클릭되도록
  ...
  handleToggle = id => {
    ...
    //    this가 바로 현재 컴포넌트다
    //    this = App.js 의 context(문맥, thread의 모든 정보를 담고 있는 것)
    ...
    // }) 원본자체를 바꿔서? 리액트가 상당히 부담스러움, 그래서 아래에선 index만 사용

    // 기존의 todoList를 nextTodoList에 복사해두기
    const nextTodoList = [...todoList];

    // 기존에 checked 값이 true -> false, false-> true로 바꾸기
    nextTodoList[index] = {
      ...selectTodo,
      checked: !selectTodo.checked
    };
